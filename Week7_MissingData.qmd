...
- **Missing Completely At Random (MCAR):** Missingness is unrelated to the parameter(s) you want to estimate; listwise deletion is unbiased under MCAR.
- **Missing At Random (MAR):** Missingness can be explained by observed variables; unbiased estimation is possible if the imputation model conditions on the right predictors.
- **Missing Not At Random (MNAR):** Missingness depends on unobserved values or the parameter itself; standard MI cannot guarantee unbiased estimates.
- **Multiple Imputation (MI):** Create *m* completed datasets by imputing plausible values, analyze each, then combine (pool) estimates.
- **Fully Conditional Specification / Chained Equations (FCS/MICE):** Iterative, variable‑by‑variable imputation using a prediction model appropriate to each variable’s type.
- **Pooled estimate ($\\bar{Q}$):** The average of estimates across the *m* completed datasets.
- **Within‑imputation variance ($\bar{V}$) & Between‑imputation variance ($B$):** Components of uncertainty combined after MI; total variance is $T = \bar{V} + B + B/m$.
- **Convergence (MI):** Stability of imputed values/statistics across iterations; checked with trace/diagnostic plots.
- **Auxiliary variables:** Observed variables related to missingness or the outcome that improve MAR plausibility and the imputation model.
- **Sensitivity analysis:** Compare conclusions across MI vs complete‑case (and other reasonable choices) to gauge robustness.
:::

:::{.callout-note collapse="false"}
## Key Concepts

- **Complete‑case analysis discards data and can bias results** unless MCAR; it is often inefficient even when unbiased.
- **Classify the missingness mechanism** (MCAR/MAR/MNAR) conceptually before choosing a method; under **MAR**, MI can recover unbiased estimates when the model conditions on the right variables.
- **MI workflow (FCS/MICE):** (1) start with incomplete data; (2) initialize missing values; (3) for each variable with missingness, fit a prediction model suited to its type (e.g., linear for continuous, logistic for binary, ordinal models for ordered factors); (4) iterate through variables; (5) repeat until **convergence**.
- **Analyze each imputed dataset and pool results.** Report $\bar{Q}$ and its uncertainty combining **within‑** and **between‑imputation** variance ($T = \bar{V} + B + B/m$).
- **Number of imputations (*m*) trades time for Monte‑Carlo error.** More imputations reduce $B/m$ but with diminishing returns; with modern hardware, ~25–100 is commonly adequate.
- **Diagnostics matter:** inspect overall and pattern‑level missingness, compare observed vs imputed distributions, and check trace plots for convergence (no systematic drifts or “split” chains).
- **Model the data you wish you had.** Use imputation models that mirror your analysis models (distribution, links, interactions) and include predictors of missingness/outcome (auxiliary variables).
- **Respect data constraints.** Ensure imputations fall in plausible ranges, maintain factor levels, and obey logical bounds/relationships.
- **Expect wider CIs after pooling.** MI appropriately reflects extra uncertainty from missing data; results are often closer to the “truth” than complete‑case under MAR.
- **Document choices** (predictors included, methods for each variable, iterations, *m*, seeds) and **run sensitivity analyses** (e.g., MI vs complete‑case) to check robustness.
:::

:::{.callout-note collapse="true"}
## Key Functions or Operators

- `VIM::aggr()` — Plot amount/patterns of missingness by variable and overall patterns.
- `VIM::marginplot()` — Visualize bivariate relationships with missingness indicated on margins.
- `mice::mice()` — Main function to run multiple imputation by chained equations (set `m`, `maxit`, methods, seed).
- `mice::mice.mids()` — Add further iterations to an existing `mids` object to check/improve convergence.
- `with()` — Fit models across imputed datasets (dispatches to `with.mids()` for `mids` objects).
- `pool()` — Combine model results across imputations to obtain pooled estimates and SEs.
- `pool.r.squared()` — Compute pooled $R^2$ for linear models fit on imputed data.
- `plot()` on a `mids` object — Convergence/trace diagnostics for selected variables/statistics.
- `exp()` — Convert logistic coefficients to odds ratios when interpreting pooled logistic models.
:::

You can view, download or pull the raw code for this content here
<https://github.com/meagan-crowther/MonashHonoursStatistics2026/blob/master/Week7_MissingData.qmd>
