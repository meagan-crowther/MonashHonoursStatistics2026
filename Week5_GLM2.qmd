---
title: "PSY4210: Generalized Linear Models 2"
author: 
  - name: Pei Hwa Goh
  - name: Joshua F. Wiley
...
- Generalized Linear Model (GLM): Extends linear models to non‑normal outcomes via a link function and distribution family.
- Link Function: Transforms the expected outcome to the linear predictor (e.g., log link for Poisson, logit link for logistic).
- Poisson Regression: GLM for count outcomes (non‑negative integers).
- Logistic Regression: GLM for binary outcomes (0/1) assuming a Bernoulli distribution.
- Incidence Rate Ratio (IRR): `exp(coef)` from a Poisson model; multiplicative change in expected count per one‑unit predictor increase.
- Odds: `p / (1 - p)`; odds ratio compares odds between groups.
- Odds Ratio (OR): `exp(coef)` from a logistic model; multiplicative change in odds per one‑unit predictor increase.
- Relative Risk (RR): Ratio of probabilities; can be estimated with a log link (e.g., Poisson with robust SEs) for binary outcomes.
- Predicted Probability: Model‑based probability on the response scale.
- Average Marginal Effect (AME): Average change in predicted probability for a one‑unit change in a predictor.
- AIC (Akaike Information Criterion): Relative measure of model fit reported for GLMs.
:::

:::{.callout-note collapse="false"}
## Key Concepts

- **GLM basics:** Choose a *family* (data distribution) and a *link function*; inference and interpretation occur on the link scale, but you can report results on the original (response) scale.
- **Poisson regression (counts):** Coefficients are on the log scale; exponentiating slopes gives IRRs. An IRR of 1.20 means a 20% higher expected count per one‑unit increase in the predictor.
- **Logistic regression (binary):** Coefficients are on the log‑odds (logit) scale; exponentiating slopes gives ORs. Intercepts refer to the reference group; convert to probabilities with `plogis()` or `predict(type = "response")`.
- **OR vs RR:** ORs and RRs differ, especially when outcomes are common; if you need RRs, fit a log‑link model (e.g., Poisson with robust SEs) and exponentiate coefficients to obtain RRs.
- **Communicating effects:** Predicted probabilities and AMEs are often more interpretable than ORs/IRRs. AMEs summarize the average change in probability for a one‑unit predictor change.
- **Uncertainty on the right scale:** Compute confidence intervals on the coefficient scale, then transform (e.g., `exp()` for IRR/OR; `plogis()` for probabilities).
- **Model output:** GLMs report deviance and AIC rather than R². Use AIC for *relative* model comparison; lower AIC indicates better fit (all else equal).
- **Practical checks:** Ensure a plausible link‑linear relationship, sufficient sample size for asymptotic inference, independence of observations, and watch for influential predictor outliers.
:::

:::{.callout-note collapse="true"}
## Key Functions or Operators

- Fitting models: `glm()` with `family = poisson()` (Poisson) or `family = binomial()` (logistic; logit link by default).
- Predictions: `predict(model, newdata, type = "response")` to obtain counts or probabilities on the original scale.
- Coefficients & intervals: `coef(model)`, `confint(model)`; transform with `exp()` (IRR/OR) or `plogis()` (probabilities from logits).
- Average marginal effects: `marginaleffects::avg_slopes(model)` for AMEs.
- Robust SEs / RRs for binary outcomes: `survey::svyglm(..., family = poisson(), design = ...)` with a log link to obtain RRs.
- Visualization: `visreg::visreg(model)` to plot effects on link or response scales.
- Distribution checks (optional): `JWileymisc::testDistribution()` to compare variable distributions to theoretical families.
:::
...
<https://github.com/meagan-crowther/MonashHonoursStatistics2026/blob/master/Week5_GLM2.qmd>
...
db[, StressHigh := as.integer(stress >= 10)]
...
:::{.callout-note collapse="true"}
Although lambda can be exactly 0, here lambda is the average number of counts / events. Although
...
That manual way of calculating is good for learning and understanding
exactly what we mean by an average marginal effect. However, it is
cumberson to do in practice. Fortunately, there is a function in the
`marginaleffects` package that will do all that for us.

Here you can see that we get the same results as we did the manual way.
In addition we get confidence itnervals and a p-value.
...
|----------------|----------------------------------------------|
| `avg_slopes()` | automatically calculate average marginal effects or average changes in probability |
