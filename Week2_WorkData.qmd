---
title: "PSY4210: Working with Data"
author: 
  - name: Joshua F. Wiley
...
- Logical Values: Boolean values (`TRUE`, `FALSE`).
- Factor Variables: Discrete variables with levels and labels.
- Wide Format: Data stored with separate columns for each time point.
- Long Format: Data stored with multiple rows per ID for each time point.
:::

:::{.callout-note collapse="false"}
## Key Concepts

- `R` can perform arithmetic operations and store results in objects we name.
- Objects can hold numbers, vectors, datasets, or results for later use.
- Vectors are created using `c()` and operations on vectors apply to each element.
- Logical operators return logical/boolean values (`TRUE`, `FALSE`) and can be combined using `&` (AND) and `|` (OR).
- Data can be subset using logical conditions. The order of operations matters when chaining subsetting steps.
- Common data types include Logical, Integer, Numeric, Character, Date, and Factor.
- Dates are often stored as character strings and need conversion using `as.Date()`.
- Factors are a way to represent discrete variables in `R` and can be created using `factor()`.
- Merging data joins datasets using the `merge()` function, with types of joins including Natural, Full Outer, Left Outer, and Right Outer.
- Reshaping data involves converting between wide and long formats using `reshape()`.
:::
...
- Assignment: `<-` for objects, `:=` for `data.table`.
- Common logical operators: `<`, `<=`, `>`, `>=`, `==`, `!=`, `%in%`, `%!in%`, `&`, `|`.
- More operators (from `extraoperators` package): `%l%`, `%le%`, `%g%`, `%ge%`, `%gl%`, `%gel%`, `%gle%`, `%gele%`, `%c%`
- Checking missing values: `is.na()`.
- Converting dates: `as.Date()`.
- Extracting date components: `weekdays()`, `months()`, `quarters()`.
- Summarizing data: `summary()`.
- Creating factors: `factor()`.
- Frequency tables: `table()`.
- Merging datasets: `merge()`.
- Checking duplicates: `anyDuplicated()`, `duplicated()`.
...
Please make sure that you have downloaded the Quarto file (.qmd) for the lecture by pulling the most recent git repo
<https://github.com/meagan-crowther/MonashHonoursStatistics2026/blob/master/Week2_WorkData.qmd>
You can follow along with the pre-recorded lecture videos for Week 2, or you can read this markdown (in HTML format) independently, and follow along by executing the code in R Studio using the markdown (qmd) file.
...
Objects in R can be almost anything. They can be a single number, a vector or set of numbers, the results from a regression, a graph, a dataset. You can have many objects at the same time in your worksapce, which can be a useful way to save results or store something for later use.
:::
...
Computers do not work quite like people or
...
is based on bits stored in 0s and 1s. 
This can mean that some numbers cannot be represented exactly.
If you try to check whether something is exactly equal to a number,
sometimes, surprisingly, it may fail. This can be 
because of how it has been presented in the computer.
:::
...
The round function has two main arguments (i.e., pieces
...
In software, logical values and logical operators are
...
:::{.callout-tip collapse="true"}
## Operators from `R` and `extraoperators` package
...
::: {.callout-tip collapse="true"}
## Reading Chained `data.table` Operations
Here, the order is: first, remove missing stress
...
using the `merge()` function. Note that merging in `R` is the same as 
joins which is the more common term for databases.
...
Merging or joins always involves two datasets. Even if you need
...
This small, hypothetical study collected data via surveys and
...
::: {.callout-tip collapse="true"}
## Details on Natural Joins

Natural joins have only the rows / observations that
...
When we write by = ID what is meant is that it should be
...
::: {.callout-tip collapse="true"}
## Details on Full Outer Join

Full outer joins include cases that are present in either dataset. In this case, in either the surveys
...
::: {.callout-tip collapse="true"}
## Details on Left Outer Join

As before, we must specify what variable(s) to
merge by so R knows how to match rows together, accomplished using
...
Knowing how many rows you expect gives you a quick way to
...
This behaviour is sometimes referred to as cross or
