...
<https://github.com/meagan-crowther/MonashHonoursStatistics2026/blob/master/Week8_Mediation.qmd>
...
## Summary & Key Points
This week we are working toward understanding what mediation is and when it can be used.
We will examine the underlying conceptual framework that underscores mediation and some cautionary tales regarding the use of mediation.

- Define mediation
- Understand the difference between moderation and mediation
- Identify approach circumstances to use mediation analysis
- Use laavan to build a mediation model
- Interpret output of mediation model 


## Structural Equation Modelling
The package we are going to be using to conduct of mediation analysis is "laavan". 
laavan is an R package that lets you test relationships between variables using Structural Equation Modeling (SEM). This is our first step in to the world of SEM.

SEM is everything we already know from regression... but with more flexibility.
If regression is the test of a single relations (X -> Y), SEM can test a whole system of relationships at once.

SEM lets you test multiple regressions at the same time, including ones with latent variables, and check how well your whole model fits the data. 

We aren't going to go too far in to the theory and practice of SEM as an approach, this is more just to explain the difference in the package and approach we are using this week.
...
## Types of mediation
Mediation analysis can be:

**1. Simple Mediation (3-variables)**
    A basic mediation model with One predictor (X), one mediatior (M) and one Outcome (Y).
    It estimates:
      - The direct effect of X -> Y
      - The indirect effect of X -> M -> T
      - The total effect of direct + indirect effects
      
**2. Moderated mediation**
  A mediation model in which one or more pathways are moderated, meaning the strength of the mediation depends on another variable (a moderator).
  This is a test of **for whom** or **under what conditions** the mediation process operates.
  The moderator can influence:
      - the a‑path (X → M)
      - the b‑path (M → Y)
      - the direct path (X → Y)
      - or the overall indirect effect 
      
**3. Parallel mediation**
  A model with multiple mediators operating at the same time, each with a relationship with the predictor and each predicting the outcome.
  In parallel mediation:
    - Mediators are not assumed to influence each other
    - Each mediator represents a distinct mechanism
    - We estimate multiple indirect effects in parallel
    
**4. Sequential (or serial) mediation**
  A model with multiple mediators arranged in a specific order, such that the effect flows through them in sequence.
  Example:
  X → M1 → M2 → Y
  This tests a multi‑step mechanism, where one mediator leads to another.
...
m_reg <- lm(stress ~ neuroticism, data = db)
summary(m_reg)
...
multiple regression. For example, this is the same model as `lm(stress ~ neuroticism + selfesteem)`:

```{r lavaan-multiple-regression}

mod_mr <- "
  stress ~ neuroticism + selfesteem
"
...
Here are some steps for us to consider as we set up for mediation.

# Preparing to conduct mediation
  **Clarify the theoretical model**
  For mediation to be meaningful, we need to consider a conceptual rationale for our model.
    - Identify the X (predictor), M (mediator), and Y (outcome)
    - Articulate why X should influence M and why M should influence Y (and why it is mediation vs moderation)
    - Ensure the proposed direction of the effects in grounded in theory or previous evidence
    - Decide if you expect partial of full mediation
    
 **Research design and temporal ordering**
  This is part of understanding what conclusions we can draw from our outcomes and casual inferences. 
    - Cross-sectional designs cannot establish temporal precedence. If using for cross-sectional must be extremely careful about inferences and over interpreting
    - Longitudinal designs are much more consistent with mediation analysis
     * **Two-wave design**: X measured at T1, M and Y measured at T2
     * **Three-wave design**: X at T1 → M at T2 → Y at T3 (ideal for temporal ordering)
     * Experimental designs allow you to expose participants to a condition of X and then measure M (which should change in response to X) and measure Y (outcome)
  **Ensure adequate measurement**
  - Use reliable, valid measures for all variables (expect when using intervention group, sex etc.)
  - Avoid single-item measures where possible
  - Ensure M is measured in a way that reflects the proposed mechanism
  
  **Sample size and power**
  Indirect effects are often smaller than direct effects, so often larger sample sizes are need
  - Power analysis calculators tools can be used (e.g., RMediation, pwrSEM)
  - Previous literature may inform the sample size required
  - If necessary, adhoc power analysis can also be run to test if you model was adequatley powered to detect the mediation effect
  
  **Data cleaning and preparation**
  - Analyse any item level missing data and deal with appropriately 
  - Score the scales as necessary
  - Create data.table with only the variables being used in the mediation

  **Examining your data**
  - Evaluate univariate normality of each variable (at the chosen timepoint you are using)
  - Identify any outliers and deal with appropriately 
  
  **Specify and run Model**
  - Create conceptual diagram of your mediation model to ensure you are specifying model correctly 
  - The path a should represent the relationship between X to M
  - The path b should represent the relationship between M to Y
  - The path c' should represent the relationship between X to Y
  - The path c (indirect) should represent ab + c'
...
Our model shows pathway a = -0.261, *p* < .001, meaning higher neuroticism is significantly associated with lower self-esteem.
Pathway b = -1.835, *p* < .001, meaning lower self-esteem predicts higher stress. 
Pathway $cp$ = 0.670, *p* < .001, meaning that, even after accounting for self-esteem, neuroticism still predicts higher stress.
Indirect effect (a + b) = 0.479, *p* <.001, our indirect effect is significant so (at least part) of the effect of neuroticism on stress
operates through self-esteem. 
Direct effect = 0.670, *p* < .001, this is the exact same as $cp$ because we only have the three variables in our model.
Total effect = 1.149, *p* <.001, this is the combination of the indirect and direct effect (0.670 + 0.479 = 1.149).
Prop = 0.417, this is the proportion of variance explain by the indirect pathway.
This means about 42% of the total effect of neuroticism on stress is explained by the indirect pathway through self‑esteem.

All together our model shows us:
"People higher in neuroticism tend to have lower self‑esteem, and this lower self‑esteem contributes to higher stress. 
Even after accounting for self‑esteem, neuroticism still predicts stress. So self‑esteem is a meaningful but partial mediator: 
it explains some, but not all, of the link between neuroticism and stress."

If the $cp$ pathway was not significant however, it would show that once we account for our mediator X no longer predicts Y.
This would be would is referred to as a "fully mediated" relationship. With the caution that this still needs to be interpreted carefully,
as there is likely still unmeasured variables here.
...
- start with your dataset of size with $N$ observations (rows)
...
The 0.05 level because we asked for 95% confidence intervals with `level = 0.95`. You can change that if you want.
