---
title: "PSY4210: Generalized Linear Models 1"
author: 
  - name: Joshua F. Wiley
...
- **Generalized Linear Model (GLM):** A framework that extends linear regression to different outcome types by combining a linear predictor, a link function, and a specified outcome distribution.
- **Linear predictor ($\eta$):** The linear combination of predictors, $\,\eta = b_0 + b_1 x_1 + \dots + b_k x_k\,$.
- **Link function ($g$):** Transforms the mean of the outcome to the linear predictor, $\,\eta = g(\mu)\,$; the inverse link returns $\,\mu = g^{-1}(\eta)\,$.
- **Normal linear regression:** A special case of the GLM with a Gaussian outcome and the **identity** link ($\mu=\eta$).
- **Residuals ($\varepsilon$):** Differences between observed and model‑predicted values, $\,\varepsilon_i = y_i - \hat{y}_i\,$.
- **Model assumptions (linear regression):** Residual **normality**, **independence** of observations, and **homogeneity of variance**.
- **Dummy coding (categorical predictors):** Representing groups with 0/1 indicators; coefficients are interpreted relative to a reference (0) group.
- **Interaction / Moderation:** When the effect of a predictor depends on another variable; tested via product terms (e.g., `x * z`).
- **Effect sizes:** **$R^2$** (variance explained) and **Cohen’s $f^2$** (overall or per‑predictor effect; rough guidelines: .02 small, .15 medium, .35 large).
- **Information criteria:** **AIC** and **BIC** for model comparison (introduced here, used more later).
:::

:::{.callout-note collapse="false"}
## Key Concepts

- **Linear regression as a GLM:** Linear models fit within the GLM framework: specify a distribution (Gaussian here), a link (identity), and a linear predictor.
- **Interpretation basics:**  
  - **Intercept ($b_0$):** Expected outcome when all predictors equal 0 (note what 0 means).  
  - **Slope ($b_j$):** Expected change in the outcome for a one‑unit increase in predictor $x_j$, holding other predictors constant.
- **Workflow:** Fit the model, **check diagnostics before interpreting** coefficients, then report estimates with **95% CIs**, $p$‑values, and effect sizes.
- **Diagnostics you should examine:**  
  - Residual distribution vs. normal (density & QQ).  
  - Residuals vs. fitted values (look for non‑constant variance and patterns).  
  - **Extreme values / potential influence** flagged via residual diagnostics.
- **Categorical predictors:** Treat categories as factors so `R` automatically dummy‑codes them; coefficients reflect **group differences** relative to the reference group.
- **Multiple regression:** Add predictors with `+`; interpret partial effects **controlling for** other variables; report **$R^2$**, adjusted $R^2$, and **Cohen’s $f^2$**.
- **Moderation / interactions:** Include interaction terms to test whether an effect varies by a second variable; interpret **simple effects** and use plots to aid interpretation.
:::

:::{.callout-note collapse="true"}
## Key Functions or Operators

- **Model fitting & summaries:** `lm()`, `summary()`, `coef()`, `confint()`, `predict()`
- **Diagnostics:** `modelDiagnostics()` then `plot()` to inspect residual normality, homogeneity, and extreme values
- **Model tests & effect sizes:** `modelTest()` (reports $R^2$, adjusted $R^2$, AIC/BIC, and Cohen’s $f^2$ overall and per predictor)
- **Visualization of models / interactions:** `visreg()` (e.g., `visreg(m, xvar = "stress", by = "sex", overlay = TRUE, gg = TRUE)`)
- **Categorical predictors:** `factor()` to ensure dummy coding in `lm()`
- **Formula operators:**  
  - `+` add a predictor  
  - `:` interaction **only**  
  - `*` main effects **and** interaction (e.g., `y ~ x * z` expands to `y ~ x + z + x:z`)
:::
...
<https://github.com/meagan-crowther/MonashHonoursStatistics2026/blob/master/Week4_GLM1.qmd>
...
Visually, it becomes more difficult to visualize, but remains possible
...
maps a value to a particular probability of occurring. Most
...
function exists that simply "undoes" whatever the link function
...
sense, all GLMs are interpreted like linear regression, which
facilitates modelling and interpretation.

Now, let us take a look at linear regression in `R` and work through
examples running and interpreting the results.
...
visreg(m, xvar = "stress", partial = FALSE, rug = FALSE, gg = TRUE) +
  theme_pubr() +
  annotate("text", x = 15, y = 3, label = "b = 0.XX, p < .0001", size = 4) + 
  ggtitle("Linear regression of neuroticism on stress",
          subtitle = "Shaded region shows 95% confidence interval.")
...
:::{.callout-note collapse="true"}
You might find this page helpful for further explanation of dummy coding: https://stats.oarc.ucla.edu/other/mult-pkg/faq/general/faqwhat-is-dummy-coding/
:::
...
two levels (e.g., male and female participants) where you might create
a new dummy coded variable where 1 = female and 0 = male
participants. This dummy coded variable is now a numeric predictor (it has
0s and 1s) and can be included in regression as usual.

The only real difference between interpreting a dummy coded variable
from a regular continuous predictor is that: (1) the 0 point is not
arbitrary, it represents one particular group (e.g., male participants)
...
Let us take a look at a simple linear regression with a categorical
predictor, `sex`. We will store these results in `m2`, for
...
db$sex <- factor(db$sex, levels = c(1, 2), labels = c("males", "females"))
...
is the mean neuroticism level in the male participants, and the
regression coefficient for `sex` is the **difference** in
neuroticism on average between male and female participants.
...
mean in the male group with the intercept, and the mean difference
as well as the p-value for `sex`).
...
- the residual variance, $\sigma_\varepsilon$, is homogeneous
...
are not siblings, we do not have repeated measures on the same person,
etc.).
...
This should be very familiar from the Data Visualization topic where
...
assumption. If the residual variance is homogeneous, we would expect
...
assumption of homogeneous variance, one would expect these lines to be
...
some predicted values than for others, known as heterogeneous
variance. The solid blue line in the middle is a smooth line at the 
...
These graph looks fairly good although the median smooth line is not great. 
The diagnostics are not perfect, but
real data never are and the degree of variation in the spread of
residuals is not extreme (but not good). We might be reasonably satisfied that the
assumption of normality and homogeneity of variance are met or at
least not terribly violated, although a case could be made that the
homogeneity assumption is violated. There also do not appear to be extreme
values in the residuals.

For the sake of example, let us look at another outcome measure. 
`selfesteem`. Again we will use `stress` as a predictor.
...
These graphs show there are three extreme values, based on
...
minimum, we would want to remove these extreme values.  We could also try a log
transformation of the outcome (although you would not do this if the normality actually 
looks ok - but let us just do it as an example anyway).
...
Even after the log transformation, the extreme values remain. Let us
...
diagnostics look much better now, although the homogeneity assumption could be 
argued to still be violated. 
...
diagnostics on purpose as there is not much point interpreting the
...
separate the plot by `sex` since it is a categorical predictor it
is easy to make separate lines for each sex group. So that the lines
...
a stastistically significant association between stress and
neuroticism, controlling for sex group. A one unit higher stress score was associated with a 
...
There 
`r ifelse(coef(summary(m3))["sexfemales", "Pr(>|t|)"] < .05, "was", "was not")`
a stastistically significant association between sex group and
neuroticism, controlling for stress. Compared to male participants, female participants had 
`r sprintf("%0.2f", round(coef(m3)[["sexfemales"]], 2))` 
[95% CI `r sprintf("%0.2f - %0.2f", round(confint(m3)["sexfemales", 1], 2), round(confint(m3)["sexfemales", 2], 2))`]
higher neuroticism scores, 
`r formatPval(coef(summary(m3))["sexfemales", "Pr(>|t|)"], d = 3, includeP = TRUE)`.
Overall, stress and sex group explained 
...
overall model. In this case, even though we can maybe tell that more of
the effect is attributable to stress, we do not have a
...
fixed effects because they do not vary across individuals. These are our model
...
intervals in brackets. Rounding to two decimal points is the default
consistent with standard APA style.

:::{.callout-note collapse="true"}

According to Cohen's (1988) guidelines, $f^2 \geq 0.02$, $f^2 \geq 0.15$, and $f^2 \geq 0.35$ represent small, medium, and large effect sizes, respectively.
:::
...
:::{.callout-note collapse="true"}
As we are focused on moderation here, we are not
...
It is very easy to include interactions in `R`. Inside of a
...
and sex on neuroticism.

```{r, results = "asis"}

mint <- lm(neuroticism ~ stress * sex, data = db)
...
visreg(mint, xvar = "stress", by = "sex", partial = FALSE, rug = FALSE,
       overlay = TRUE, gg = TRUE) +
  theme_pubr() +
...
and sex group. It is likely that if
...
show what could be interpreted if there was a significant
interaction, we will proceed.
There was a significant simple effect of stress
on neuroticism, such that for male participants (i.e., when sexfemales =
0), each one unit higher stress was associated with 0.22 higher
neuroticism [95% CI 0.08 - 0.36], p = .003 and this was a small
effect, $f^2 = 0.07$. There was no simple effect
of sex on neuroticism when stress was held at 0, (b = 1.37, [95%
CI -0.22 - 2.95], p = .091) and the effect size was small, $f^2 = 0.02$. 
The interaction was not significant, and a graph of the
interaction showed that the pattern was that the association between
stress and neuroticism was about the same in male and female participants.
...
continuous self esteem instead of sex.
...
We can plot the results with `visreg()`. Although self esteem is
...
`breaks` argument to specify specific self esteem scale points we want it to plot the
...
Overall we can interpret that there is no statistically significant
interaction between self esteem and stress on neuroticism (p = .153). 
The association between stress and neuroticism does not depend on self esteem.

Note that with continuous predictors, unless they were mean centered,
the default simple effects are not very useful. For example we would
say that the simple effects show that each one unit higher stress is
associated with -0.01 lower neuroticism, when selfesteem = 0. Although a
technically accurate interpretation, no one scored 0 on selfesteem in
this dataset (because the way we totaled it is impossible) and so we probably do not want to extrapolate to
self esteem = 0. This is because the default simple effects from 
